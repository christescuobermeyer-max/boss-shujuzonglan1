...
function withDefaultBaseList(bases: string[]) {
  return bases.length > 0 ? bases : ["https://8-136-183-128.sslip.io"];
}

function getOpenApiBases() {
  const env = getEnv();
  return Array.from(
    new Set([
      ...withDefaultBaseList(env.openApiBases),
      ...env.openApiInsecureTlsBases
    ])
  );
}

function shouldAllowInsecureTlsForUrl(upstreamUrl: string) {
  const env = getEnv();
  const url = new URL(upstreamUrl);
  return env.openApiInsecureTlsBases.some((base) => {
...
export async function fetchOpenApiJson<T>(
  ...
  if (!token) {
    console.error(`${params.logLabel} open api token missing`, {
      ...
      envName: "CHENGSHANG_OPEN_API_TOKEN"
    });
    return {
      ok: false,
