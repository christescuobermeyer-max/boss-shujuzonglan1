...
const ELEME_AMOUNT_KEY = "代运营结算金额";
...
function normalizeAmount(value: unknown) {
  ...
  const bracketNegative = cleaned.startsWith("(") && cleaned.endsWith(")");
  const matched = cleaned.replace(/[()]/g, "").match(/-?\d+(\.\d+)?/);
  if (!matched) return 0;

  const amount = Number(matched[0]);
  if (!Number.isFinite(amount)) return 0;
  return bracketNegative ? -Math.abs(amount) : amount;
}
...
export function extractDailyPointAmount(
  ...
  if (platform === "eleme") {
    const targetKey = normalizeKey(ELEME_AMOUNT_KEY);
    for (const entry of entries) {
      if (normalizeKey(entry.key).includes(targetKey)) {
        return normalizeAmount(entry.value);
      }
    }
    return 0;
  }
  ...
  for (const keyword of AMOUNT_KEYWORDS) {
    for (const entry of entries) {
      if (!entry.key.includes(keyword)) continue;
