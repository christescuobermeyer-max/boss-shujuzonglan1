function parseMonth(month: string) {
  ...
  if (!matched) throw new Error(`无效月份: ${month}`);
...
export function buildRecentMonthOptions(currentMonth: string, count: number) {
  return Array.from({ length: count }, (_, index) => shiftMonthValue(currentMonth, -index));
}

export function getNextRotationMonth(months: string[], currentMonth: string) {
  const currentIndex = months.indexOf(currentMonth);
  if (currentIndex === -1 || months.length === 0) return months[0] ?? currentMonth;
