...
import { buildDashboardMetricCards } from "@/lib/stats/dashboard-metric-cards";
...
import { useLatestOnlineShopSummary } from "@/lib/stats/use-latest-online-shop-summary";
...
export function MonthlyStatsDashboard() {
  ...
  const latestOnlineShopSummary = useLatestOnlineShopSummary();
  ...
  const terminationTotal = stats.operatorTerminationTrend.reduce((sum, item) => sum + Number(item.count ?? 0), 0);
  const wuhanShopCount = Number(stats.salesCityShopTrend.find((item) => item.name === "武汉")?.count ?? 0);
  const yichangShopCount = Number(stats.salesCityShopTrend.find((item) => item.name === "宜昌")?.count ?? 0);
  ...
  const metricCards = buildDashboardMetricCards({ loading, stats, terminationTotal, wuhanShopCount, yichangShopCount });
  ...
  return (
    <main className="dashboard-shell">
      ...
      <DashboardOverviewSection
        ...
        yichangMonthlyPointSummary={stats.yichangMonthlyPointSummary}
